...
Note: This repo focuses on synthesizable design + integration (bitstream/report generation). Simulation testbenches exist in companion verification repos / versions of the project (tile scan, FC regression, AXI-lite TBs, etc.).
...
- Full AXI4-Lite “PS ↔ accelerator” shell: register map + load windows (X/W/B) + control + readback (logits/cycles)
...
Tools
- Vivado 2024.1 (Linux recommended)

Target device / board class
- xc7z020clg400-1 (PYNQ-Z2 / Zynq-7020 class)
...
2. Ensure the Block Design contains:
   - processing_system7
   - AXI interconnect
...
The peripheral is memory-mapped at a base address chosen in Vivado Address Editor (example base: 0x4000_0000). The RTL decodes offsets (low address bits). Add your base in software.
...
j ranges 0..9. Each logit is signed ACC_BITS (default 40-bit).
...
0x3030                        W        COMMIT staged lanes as one tile write
k ranges 0..TILE_LEN-1 (typically 8). Tile index t ranges 0..N_TILES-1.
...
Recommendation: always write values sign-extended into the 32-bit AXI word (int16 → int32 sign extension in software).
...
3) For each inference case:
   ...
   * pulse START
   * poll STATUS.DONE
   * read logits from the LOGITS window
   * optionally read CYCLES

10. Timing closure note (demo realism)

This design is functionally verified in simulation; for hardware reliability it must also be timing-clean at the target clock.

If WNS is negative at 100 MHz, common low-risk fixes:

* add AXI register slice / interconnect pipelining in Block Design
* pipeline AXI read/write response inside msdf_axi_lite_regs (AXI-Lite allows variable latency)
